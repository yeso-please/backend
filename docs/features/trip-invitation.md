...
> **2026-09-24 재작업** — [공동 일정 정책](../mvp/decisions.md#2026-09-24-여행공동-일정-정책)에 따라 비회원 참여(guest session·손님 온보딩)와 VIEW/EDIT 권한을 제거했다. 초대는 로그인·설문을 마친 회원이 수락하고(`POST /api/invites/{token}/accept`), 참여자는 모두 동등하며, 공유 링크는 읽기 전용이다. 폐기는 `DELETE`다. 현재 계약은 [여행 API 4장](../api/trip.md#4-초대공유)이 기준이며 아래 본문의 비회원·권한 설명은 이력이다. 친구 직접 초대(4-6~4-8)는 친구 기능 이후다.
