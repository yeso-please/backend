...
| `trip_plans`, `trip_participants`, `trip_stops` | 생성자, 날짜, transport, 지역 선택 방식·밀도, 요청자 취향 스냅샷, visit date/order/type |
...
- 관광지 점수는 **요청한 사람** 한 명의 취향 벡터로 구한다. 참여자 취향을 평균하지 않는다. 제외 조건은 판정 가능한 것(`물놀이` 분류, `야간 이동`)만 적용한다(2026-09-25). 제외 조건도 요청자의 것을 적용한다.
...
→ 날짜별 가용 시간(첫날 12:00, 마지막 날 17:00, 당일 12:00~20:00)
→ 점심/저녁 슬롯 선점 → 가까운 후보 순열 생성
→ 단순 체류 슬롯+이동 추정으로 feasibility 검사
→ 상위 일정 선택 → 규칙 제목(LLM은 선택) → 초안 반환
```

- RELAXED는 하루 최대 4곳, PACKED는 최대 6곳이며 자동 생성의 목표다(수동 편집은 가용 시간만 검사). 슬롯을 채우기 위해 제약을 깨지 않는다. 선택 밀도보다 적게 배치되면 `DENSITY_TARGET_NOT_MET`을 반환한다. 하루 최소 1곳을 못 채우는 날이 있을 때만 `COURSE_INSUFFICIENT_CANDIDATES`(422)다.
- 체류시간은 포토/전망 등 60분, 일반 관광지 90분, 대형 문화시설·레포츠 120분의 카테고리 기본값이다. 모두 `estimated`다.
- 점심은 12:00~13:00, 저녁은 18:00~19:00 고정이다(사용자가 바꾸지 않는다). 편집(추가·교체·삭제·이동)마다 전체 일정을 재검사한다.
- 이동은 Haversine 거리와 보수 계수로 1차 추정한다. 외부 길찾기 실패 정책 확정 전에는 항상 근사 경고를 붙인다.
- 목적 함수는 `취향 적합 + 유형 다양성 - 이동시간 - 과밀`이다. 날짜 범위, 중복, 식사 침범은 감점이 아니라 불가능 조건이다.
...
- 카카오 Local `FD6`는 직전 관광지 좌표, 기본 반경 5km, 거리순 fallback이다. API 결과는 맛·대표성·대표 메뉴를 보장하지 않으므로 그런 문구를 만들지 않는다.
