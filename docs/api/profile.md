...
- 질문·채점 기준: [온보딩 질문 계약 `demo-mbti-v1`](../mvp/onboarding-questionnaire.md)
...
- 계약 상태: draft
- 목적: 여행기의 `FRIENDS` 공개 범위와 친구 여행 지도를 위한 상호 친구 관계. 전체 지도 공개·피드·댓글·좋아요는 없다.
...
| ID | 항목 | 현재 초안 |
|---|---|---|
| F-2 | 차단 | mvp에는 `BLOCKED` 상태가 있지만 링크 방식에서는 원치 않는 요청이 오지 않는다. MVP는 친구 끊기(2-10)만 두고 차단은 제외하는 초안이다 |
