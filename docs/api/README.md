...
| 링크 URL | 초대·공유 링크는 **프론트 라우트**로 보낸다(예: `https://{프론트}/invite/{token}`, `/shared/{token}`). 프론트가 해당 API를 `credentials: 'include'`로 호출한다. share session cookie가 전송되려면 프론트와 API가 같은 site(등록 도메인)여야 한다 |
...
요청에 JWT와 share session이 함께 있으면 경로가 정한 주체를 쓴다. `/api/shared/**`는 share session, 그 밖은 JWT다.
...
- 식당 `selectionToken`(`rs_`)은 비밀이 아니라 서버 비밀키로 서명한 값이며 DB에 저장하지 않는다([5-5](trip.md#5-5-식당-추천)).
