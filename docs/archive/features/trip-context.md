...
> **2026-09-24 재작업** — [공동 일정 정책](../mvp-decisions-2026-09-20.md)에 따라 여행은 만드는 순간 기간을 차지한다(확정 단계 없음). 중복 검사는 내가 만들었거나 참여 중인 모든 여행을 본다. 여행 생성은 최초 설문 완료가 필요하고(`409 ONBOARDING_REQUIRED`), 생성 후 날짜는 바꿀 수 없으며(`PATCH`는 이동수단·출발지만), 내 여행 목록·참여자 목록·탈퇴(마지막 참여자면 여행 삭제)를 추가했다. 현재 계약은 [여행 API 3장](../../api/trip.md#3-여행-context지역)이 기준이며 아래 본문의 확정 여행 기준 설명은 이력이다.
